...
knnpred_tr = class::knn(train = KNNtest, test = KNNtrain, cl = KNNdefault_tr, k = 10, prob = TRUE)
...
ggroc(roc_qda)
```
Training error rate: 22.96%
Test error rate: 30.67%
Sensitivity: 74.2%
Specificity: 55%
...
The classifier I would reccomend is LDA. Looking at the error rates of the testing and training it seems to be about 8-10% lower than QDA and the optimal KNN. The sensitivity is higher than both QDA and KNN with 87.6% so it is more likely to find a true positive than the others.  The AUC of the ROC is highest in LDA which means that the model predicts it most correctly compared to the other three. Although the specificity is lower then QDA with it finding a true negative a less than the QDA the classifier LDA is only lower by less than 2% so in all the LDA seems the most accurate overall. 
