...
lines(predict(lo))
```
Looking at the table it seems that k = 26 is the optimal with the error rate of 31.4%

```{r}
set.seed(123)
knnpred = class::knn(train = KNNtrain, test = KNNtest, cl = KNNdefault, k = 26, prob = TRUE)
table(knnpred, test2$Default) -> knntab
knntab

knnpred_tr = class::knn(train = KNNtest, test = KNNtrain, cl = KNNdefault_tr, k = 26, prob = TRUE)
...
```
Training error rate: 31.4%
Testing error rate: 27.7%
Sensitivity: 94.3%
Specificity: 6.6%
...
The classifier I would reccomend is LDA. Looking at the error rates of the testing and training it seems to be about 8-10% lower than QDA and the optimal KNN. The sensitivity is higher than QDA with 87.6% so it is more likely to find a true positive, and the specificity is higher in KNN. Although the two have higher sensitivity or higher specificity LDA ended up with the higher AUC of ROC because it has the better prediction overall of the true values. Which leads to LDA being the best classifier. 
